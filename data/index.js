...
export async function getAllDocSlugs(doc) {
  const data = await getClient().fetch(
    `*[_type == "${doc}" && !(_id in [${queries.homeID}, ${queries.shopID}, ${queries.errorID}, ${queries.journeyHomeID}, ${queries.tagsHomeID}]) && wasDeleted != true && isDraft != true]{ "slug": slug.current }`
  );
  return data;
}
